...
python3 -c "import tensorflow; import cv2; import numpy; import dlib" &> /dev/null
if [ $? -ne 0 ]; then
...
python3 -c "from config import get_config; c,_ = get_config(); exit(0 if c.f != 650 else 1)" &> /dev/null
if [ $? -ne 0 ]; then
