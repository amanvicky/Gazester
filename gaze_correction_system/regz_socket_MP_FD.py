...
Uses TensorFlow 2.x with Keras for neural network inference
"""
...
class VideoReceiver:
    ...
    def face_detection(self, frame, shared_v, lock):
        ...
        face_detect_gray = cv2.resize(gray,
                                      (self.face_detect_size[0],
                                       self.face_detect_size[1]))
    ...
    def start_recv(self, shared_v, lock):
        ...
        while True:
            ...
            self.video_recv_hd_thread = Thread(
                target=self.face_detection,
                args=(frame, shared_v, lock)
            )
...
class GazeRedirectionSystem:
    ...
    def _load_model(self, eye):
        with tf1.Graph().as_default() as g:
            with tf1.name_scope('inputs'):
                ...
                phase_train = tf1.placeholder(
                    tf.bool,
                    name='phase_train'
                )

            # Build model using Keras
            flx_model = model.FLXGaze(conf)
            img_pred, _, _ = flx_model(input_img, input_fp, input_ang,
    ...
    def get_inputs(self, frame, shape, pos="L", size_I=[48, 64]):
        ...
        for i, d in enumerate(FP_seq):
            resize_x = int((shape.part(d).x - LT_coor[1]) *
                          size_I[1] / ori_size[1])
            resize_y = int((shape.part(d).y - LT_coor[0]) *
                          size_I[0] / ori_size[0])

            ach_map_y = np.expand_dims(
                np.expand_dims(np.arange(0, size_I[0]) - resize_y, axis=1),
                axis=2
            )
            ach_map_y = np.tile(ach_map_y, [1, size_I[1], 1])

            ach_map_x = np.expand_dims(
                np.expand_dims(np.arange(0, size_I[1]) - resize_x, axis=0),
                axis=2
            )
            ...
            if i == 0:
                ...
                ach_map = np.concatenate(
                    (ach_map, ach_map_x, ach_map_y), axis=2
                )
    ...
    def shifting_angles_estimator(self, R_le, R_re, shared_v, lock):
        ...
        R_w = (Rw_lt[0] + pos_remote_head[0],
               Rw_lt[1] + pos_remote_head[1])
        ...
        if eye_dist < 1:
            eye_dist = 1  # Prevent division by zero
    ...
    def flx_gaze(self, frame, gray, detections, shared_v, lock,
    ...
    def redirect_gaze(self, frame, shared_v, lock):
        ...
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        face_detect_gray = cv2.resize(gray,
                                      (self.size_df[0], self.size_df[1]))
        detections = self.detector(face_detect_gray, 0)

        rg_thread = Thread(
            target=self.flx_gaze,
            args=(frame, gray, detections, shared_v, lock)
        )
        rg_thread.start()
        return True
